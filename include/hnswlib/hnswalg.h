...
//                    if (candidate_id == 0) continue;
    ...
    */
    ...
    void addPointHybridBatch(const void *datapoint, hnswlib::labeltype label, HybridBatchManager* gpu_manager, int local_index, int beam_ef) {
        ...
        if (currObj == -1) {
            addPoint(datapoint, label);
            return;
        }

        // if (label % 1000 == 0) std::cout << "[Checkpoint B] Starting Greedy Descent" << std::endl;

        // cpu greedy descent
        // The cpu moves from the top layer down to layer 1
        // It will hop to the closest neighbor on each layer
        dist_t curdist = fstdistfunc_(datapoint, getDataByInternalId(currObj), dist_func_param_);
        ...
        for (int level = maxlevel_; level > 0; level--)
        {
            ...
            while (changed) {
                // if (greedy_ops++ > 10000) {
                //     std::cout << "!!! INFINITE LOOP DETECTED IN GREEDY DESCENT at label " << label << std::endl;
                //     break; 
                // }
                changed = false;

                /// get the height of the ndoe we are currently standing on
                int node_level = element_levels_[currObj];

                // if the node doesn't exist at this height, we have to drop down
                if (level > node_level) break;

                char* linkListPtr = linkLists_[currObj];

                // if node has no links, don't hop
                if (linkListPtr == nullptr) {
                    break;
                }

                // get the neighbors of the current node on this layer
                tableint *data = (tableint * ) (linkLists_[currObj] + (node_level - level) * size_links_per_element_);

                // make sure that layer data isn't null
                if (data == nullptr) {
                    break;
                }
                
                int size = *data;
                if (size < 0 || size > (M_ * 2)) break;

                for (int i = 1; i <= size; i++)
                {
                    tableint cand = data[i];

                    // if candidate id is bad, just kskip
        ...
        while (!q.empty() && candidate_pool.size() < gpu_manager->candidates_per_query) {
            tableint cand = q.front();
            q.pop();

            // get the pointer to the node's neighbors on layer 0
            tableint* neighbors = (tableint*)(data_level0_memory_ + cand * size_data_per_element_);
            int size = *neighbors; 
    ...
    void linkBatchElement(tableint query_id, const std::vector<int>& candidate_ids, const std::vector<float>& gpu_distances) {
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;

        // put all gpu resultsi nthe que
        for (size_t i = 0; i < candidate_ids.size(); i++) {
            // only put the valid nodes to prevent bad links
            if (candidate_ids[i] >= 0 && candidate_ids[i] < cur_element_count) {
                top_candidates.push(std::make_pair((dist_t)gpu_distances[i], (tableint)candidate_ids[i]));
            }
        }

        const void* datapoint = getDataByInternalId(query_id);

        mutuallyConnectNewElement(datapoint, query_id, top_candidates, 0, false);
    }
    ...
    void preRegisterHybridNode(labeltype label, const void *datapoint) {
        ...
        memset(data_level0_memory_ + label * size_data_per_element_ + offsetLevel0_, 0, size_data_per_element_);

        // copy the vector data into the initialized memory
        memcpy(data_level0_memory_ + label * size_data_per_element_ + offsetData_, datapoint, data_size_);
 
        // write the label to memory
        memcpy(getExternalLabeLp(label), &label, sizeof(labeltype));
        
        // register label map
        label_lookup_[label] = label;

        // increment the global count in order
        cur_element_count++;
    }
};
}  // namespace hnswlib
